...
<body>
    ...
    <nav class="sidebar">
        <div class="container-fluid">
            ...
            <ul class="navbar-nav">
                <li class="nav-item">
                    <a id="pinjamanLink" class="nav-link" href="pinjaman"><i class="fas fa-money-bill-wave"></i> PINJAMAN</a>
                </li>
                <li class="nav-item">
                    <a id="angsuranLink" class="nav-link" href="angsuran"><i class="fas fa-calendar-alt"></i> ANGSURAN</a>
                </li>
            </ul>
        </div>
    </nav>
    ...
    <div class="main-content">
        <div class="table-container">
            <h2>Data Pinjaman</h2>
            <p>Data Peminjaman Anggota.</p>
            <a href="pinjaman/create" class="btn btn-success mb-3">Tambah Pinjaman</a>
                    ...
                    <?php if (!empty($pinjaman)) : ?>
                        <?php foreach ($pinjaman as $row) : ?>
                            <tr>
                                ...
                                <td><?= number_format($row['jumlah_pinjaman'], 2); ?>
                                <td><?= $row['persentase_bunga']; ?>%</td>
                                <td><?= number_format($row['bunga'], 2); ?></td>
                                <td><?= number_format($row['total_pembayaran'], 2); ?></td>
                                <td><?= $row['tanggal_pinjaman']; ?></td>
                                <td><?= $row['tenor']; ?></td>
                                <td><?= $row['status_label']; ?></td>
                                <td>
                                    <a href="pinjaman/edit/<?= $row['id_pinjaman']; ?>" class="btn btn-warning btn-sm">Edit</a>
                                    <a href="pinjaman/delete/<?= $row['id_pinjaman']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?');">Hapus</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else : ?>
                        <tr>
                            <td colspan="7">Tidak ada data pinjaman.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
